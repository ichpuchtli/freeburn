...
#include <board.h>				/* Include device specific declareation file here */
#include <pio/pio.h>
#include <spi/spi.h>
...
/* SPI Functions						                                 */
/*-----------------------------------------------------------------------*/
const Pin POWERUPuSDN = {1 << 25, AT91C_BASE_PIOA, AT91C_ID_PIOA, PIO_OUTPUT_1, PIO_DEFAULT};
...
void mmc_spi_init(void) {
	...
	//Turn on VCC
	PIO_Configure(&POWERUPuSDN, 1);
	PIO_Clear(&POWERUPuSDN);    

	//Configure SD pins
	PIO_Configure(pinsSd, PIO_LISTSIZE(pinsSd));

	//Enable SPI
	SPI_Configure(BOARD_SD_SPI_BASE,
        AT91C_ID_SPI1,
        AT91C_SPI_MSTR |
    ...
    SPI_ConfigureNPCS(BOARD_SD_SPI_BASE,
        0,                  // NPCS0
        AT91C_SPI_NCPHA |   // Mode 0 (CPOL=0,CPHA=0)
        (255 << 8) |        // SCBR: SPCK=MCK/SCBR //8 or 64
        (1 << 16) |         // DLYBS: Delay from NPCS to SPCK in MCK cycles
        (1 << 24) |         // DLYBCT: Delay between consec xfers
        AT91C_SPI_BITS_8    // 8-data bits
    ); 

    SPI_Enable(BOARD_SD_SPI_BASE);
...
)
{
    ...
    {
        SPI_Write(BOARD_SD_SPI_BASE, 0, buff[i]);
        while(!SPI_IsFinished(BOARD_SD_SPI_BASE));     // Wait for write to complete

#ifdef DEBUG
		debug_printf("MMCSPI DEBUG: tx buff - %x\n", buff[i]);
#endif		
    }
}
...
#ifdef DEBUG
		debug_printf("MMCSPI DEBUG: rx buff - %x\n", buff[i]);		
#endif
